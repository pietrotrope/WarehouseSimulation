...
class Agent:
    ...
    def get_task(self):
        ...
        if task is None:
            ...
            if abs(self.env.key_to_raster(route_to_pod[-1])[0][0] - self.env.key_to_raster(route_to_ps[0])[0][
                0]) > 2 or abs(
                    self.env.key_to_raster(route_to_pod[-1])[0][1] - self.env.key_to_raster(route_to_ps[0])[0][1]) > 2:
                print()
                print(route_to_pod[-1])
                print(route_to_ps[0])
                print()
